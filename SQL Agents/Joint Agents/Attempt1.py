...
class InstagramPostsAgent:
    ...
    def generate_sql_query(self, query):
        chat_template = ChatPromptTemplate.from_messages([
            SystemMessage(content="Você é um agente especializado em gerar consultas SQL para dados de posts do Instagram."),
            HumanMessage(content=f"""Gere uma consulta SQL para obter dados de posts do Instagram com base na pergunta: '{query}'. 
    ...
    def analyze_data(self, data):
        chat_template = ChatPromptTemplate.from_messages([
            ...
            HumanMessage(content=f"Analise os seguintes resultados dos posts do Instagram: {data}")
        ])
        messages = chat_template.format_messages()
...
class MainAgent:
    ...
    def handle_query(self, user_query):
        ...
        if classification == "instagram posts":
            ...
            return self.consolidate_responses(instagram_response, meta_response)
        elif classification == "geral":
            # Resposta genérica para saudações ou perguntas que não envolvem o banco de dados
    ...
    def consolidate_responses(self, instagram_response, meta_response):
        consolidated = f"Análise Instagram Posts: {instagram_response}\nAnálise Meta Ads: {meta_response}"
        print(f"Consolidated Response: {consolidated}")
        return consolidated
